...
function Users({BACE_URL, URL_LOGIN, URL_MANAGEMENT_LOGIN}) {
    const [Users, setUsers] = useState({})
    const [disableHandlere, setDisableHandlere] = useState(false)
    const tokenUser = JSON.parse(localStorage.getItem("idUser"))
    ...
    const handlerEdit = (e)=>{
        ...
        axios.put(`${BACE_URL}/users/${tokenUser}.json`, value)
            .then(response => {
                setEdit(!edit)
    ...
    const handlerDel =()=>{
        axios.delete(`${BACE_URL}/users/${tokenUser}.json`)
        .then(response => {
            setDelete(!Delete)
    ...
    useEffect(() => {
        axios.get(`${BACE_URL}/users/${tokenUser}.json`)
            .then(req=>{
                if(req.status === 200){
                    setUsers(
                        {
            ...
            <ul className={edit ? "active_box": Delete ? "active_box": ""}>
                <li>
                    ...
                    <div className="items__users">
                        ...
                        <ItemsUserControl deleteClick={deleteClick} editClick={editClick} singOutClick={singOutClick} disableHandlere={disableHandlere}/>
                    </div>
                </li>
            </ul>
            {/* if  click edit*/}
